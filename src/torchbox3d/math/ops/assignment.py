...
def uniform(cuboids_a: Tensor, cuboids_b: Tensor, **kwargs: int) -> Tensor:
    """Compute axis-aligned 3D IoU."""
    return torch.ones_like(cuboids_a[:, 0])
...
# def _choose_topk(
#     input_detached: Tensor,
#     target: Tensor,
#     cart: Tensor,
#     mask: Tensor,
#     panoptics: Tensor,
#     all_foreground: Tensor,
#     k: int,
#     additive_smoothing: float,
# ) -> Tuple[Tensor, Tensor, Tensor, Tensor, Tensor]:
#     target = target.type_as(input_detached)
...
#     ).squeeze(1)
#     gts = decode_range_view(
#         target, cart=cart, enable_azimuth_invariant_targets=True
#     ).squeeze(1)

#     affinities = torch.zeros_like(target[:, 0:1])
#     foreground_mask = torch.zeros_like(target[:, 0:1])
#     num_points = torch.zeros_like(target[:, 0:1])

#     for i, _ in enumerate(panoptics):
#         panoptic_mask = (
#             F.one_hot(panoptics[i]).permute(0, 3, 1, 2)[:, 1:].squeeze(0).bool()
#         )
#         for instance_mask in panoptic_mask:
#             if instance_mask.sum() == 0:
#                 continue

#             pd = pds[i : i + 1].masked_select(instance_mask).view(7, -1).t()
#             gt = gts[i : i + 1].masked_select(instance_mask).view(7, -1).t()

#             dists = cast(Tensor, torch.linalg.norm(pd[:, :3] - gt[:, :3], dim=-1))

#             likelihoods = torch.exp(-dists)

#             k_actual = min(k, len(likelihoods))
#             likelihoods, indices = likelihoods.topk(k=k_actual)
#             likelihoods = torch.zeros_like(dists).scatter(0, indices, likelihoods)
#             affinities[i : i + 1].masked_scatter_(
#                 instance_mask, likelihoods.type_as(affinities)
#             )
#             foreground_mask[i : i + 1].masked_scatter_(
#                 instance_mask, likelihoods.bool().type_as(affinities)
#             )
#             num_points[i : i + 1].masked_scatter_(
#                 instance_mask,
#                 likelihoods.bool().type_as(affinities) * k_actual,
#             )
...
#         (num_points.double() + additive_smoothing).reciprocal().nan_to_num(posinf=0.0)
#     )
#     return (
#         affinities,
#         background_mask,
...
def _compute_dists(input_detached: Tensor, target: Tensor, ord: int = 2) -> Tensor:
    dists = cast(
        Tensor,
        torch.linalg.norm(
            input_detached[:, :3] - target[:, :3], ord=ord, dim=1, keepdim=True
        ),
    )
    return dists


def _center(cuboids_a: Tensor, cuboids_b: Tensor, **kwargs: int) -> Tensor:
    dists = _compute_dists(cuboids_a, cuboids_b)

    # Compute thresholds.
    c1 = (dists <= 0.5).type_as(cuboids_a)
    c2 = (dists <= 1.0).type_as(cuboids_a)
    c3 = (dists <= 2.0).type_as(cuboids_a)
    c4 = (dists <= 4.0).type_as(cuboids_a)

    affinities = (c1 + c2 + c3 + c4) / 4.0
    return affinities
